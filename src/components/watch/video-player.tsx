      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4 text-sm text-zinc-300 shadow-xl shadow-black/30 sm:flex-row sm:items-center sm:justify-between sm:rounded-3xl">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-zinc-400">
            Other servers are disabled for a cleaner playback experience. The
            player runs in a restricted iframe sandbox to reduce popups,
            top-page redirects, and invasive third-party behavior.
          </p>
        </div>
        <button
          type="button"
          onClick={() => setReloadKey((key) => key + 1)}
          className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs font-semibold text-emerald-300 transition hover:bg-emerald-500/20 focus:outline-none focus:ring-2 focus:ring-emerald-500">
