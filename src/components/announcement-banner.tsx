...
export default function AnnouncementBanner() {
  ...
  const content = (
    <>
      <strong>{title}</strong>
      {message ? <span className="ml-2 text-white/75">{message}</span> : null}
    </>
  );
  return (
    <div className="border-b border-red-500/20 bg-red-950/40 px-4 py-2 text-center text-sm text-white">
